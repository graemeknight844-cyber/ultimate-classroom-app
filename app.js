...
// ============================================================================
// SELF-CONTAINED QUIZ INTERFACE WIRING CORE
// ============================================================================

// A global safety check to ensure quizState exists
...
// This self-running function automatically attaches actions to your HTML buttons
(function connectQuizControls() {
  // We use standard intervals to wait safely until the page is fully drawn
  const attachmentCheck = setInterval(() => {
    const addBtn = document.getElementById('addQuestionToBankBtn');
    const launchBtn = document.getElementById('launchQuizBtn');
    const emergencyBtn = document.getElementById('emergencyEndQuizBtn');

    // If the buttons aren't loaded in the browser yet, keep waiting
    if (!addBtn || !launchBtn) return;

    // Stop waiting once we find them
    clearInterval(attachmentCheck);

    // --- BUTTON 1: ADD TO QUIZ DECK ---
    addBtn.onclick = function() {
      ...
      // Read which radio button index is marked correct
      let correctIdx = 0;
      for (let i = 0; i < radios.length; i++) {
        if (radios[i].checked) { correctIdx = i; break; }
      }

      // Package up the card data structure cleanly
      const quizCard = {
        question: qInput.value.trim(),
        options: [
          opt0?.value.trim() || "Option A",
      ...
      // Push into the queue list
      quizState.plannedQueue.push(quizCard);

      // Clear the text boxes so you can type another question card quickly
      qInput.value = "";
      if (opt0) opt0.value = "";
      if (opt1) opt1.value = "";
      if (opt2) opt2.value = "";
      ...
      // Refresh the visual counter look
      const countBadge = document.getElementById('quizBankCountBadge');
    ...
    // --- BUTTON 2: LAUNCH AUTOMATED QUIZ ---
    launchBtn.onclick = function() {
      if (quizState.plannedQueue.length === 0) {
      ...
      // Load up the first question card into our active view engine
      ...
      // Fire off the real-time transmission payload down the pipe to student interfaces
      ...
      // Run a simple 3-2-1 overlay loop timer
      let count = 3;
